...
public class InMemoryUserRepository implements UserRepository {
    ...
    public User save(User user) {
        ...
        else repository.computeIfPresent(user.getId(), (k, v) -> user);
        log.info("save {}", user);
        return user;
    }
    ...
    public User getByEmail(String email) {
        ...
        return repository.values().stream().filter(u -> u.getEmail().compareToIgnoreCase(email) == 0).findAny().orElse(null);
    }
}
